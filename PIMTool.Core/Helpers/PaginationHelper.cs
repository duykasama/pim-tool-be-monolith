...
{
    public static async Task<PaginatedResult> BuildPaginatedResult<T, TDto>(IMapper mapper, IQueryable<T> source, int pageSize, long pageIndex)
    {
        ...
        var results = await source.Skip(((int)pageIndex - 1) * pageSize)
